...
export default function ContactUs() {
  ...
  function sendEmail(e) {
    ...
    emailjs.sendForm('gmail', 'Admision_Santisima', e.target, `${credentials.correosantisima}`)
      .then((result) => {
    ...
    useEffect(()=>{
        fetch('https://escuelasespeciales.cl/curso/list?ordering=nombre&id_colegio=1')
    ...
    <div className="AdmisionFormContainer">
        <form id="AdmisionWidth" onSubmit={sendEmail} className="form">
            ...
            <span className='AdmisionForm-input-login'>
                O contactanos directamente llamando al:<p>(+569) 62382642</p>
            </span>
        </form>
    </div>
    );
