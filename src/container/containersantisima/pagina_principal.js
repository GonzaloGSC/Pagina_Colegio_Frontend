...
import Carrusel from '../../components/eventos/eventossantisima';
...
class Pagina_principal extends React.Component {
    ...
    componentDidMount(){
        fetch('https://escuelasespeciales.cl/noticia/list?ordering=-id&id_colegio=1')
            .then(response => response.json())
            .then(data => this.setState({noticia:data}))
            .catch(error => {console.log(error)})
            

        fetch('https://escuelasespeciales.cl/info_destacada/list?ordering=-id&id_colegio=1')
            .then(response => response.json())
    ...
    render(){
        ...
        if(this.state.info.length === 0 && this.state.noticia.length !== 0){
            return(
                <>
                    <Separador title="EVENTOS" separador={"SeparadoresBackgroundS"}/>
                    <Carrusel/>
                    <Separador title='ACTIVIDADES' separador={"SeparadoresBackgroundS"}/>
                        <div className="CardContainer">
                            <div className='CardWrapper'>
                                <div className='CardColumn'>
                                    <h2 id="SubtituloS1">No hay Actividades aún</h2>
                                </div>
                            </div>
                        </div>
                    <Separador title="NOTICIAS" separador={"SeparadoresBackgroundS"}/>
                        <div className="CardContainer">
                            <div className='CardWrapper'>
                                <div className='CardColumn'>
                ...
                <>
                    ...
                    <Separador title="NOTICIAS" separador={"SeparadoresBackgroundS"}/>
                        <div className="CardContainer">
                            <div className='CardWrapper'>
                                <div className='CardColumn'>
                                    <h2 id="SubtituloS1">No hay Noticias aún</h2>
                                </div>
                            </div>                    
                        </div>
                </>
